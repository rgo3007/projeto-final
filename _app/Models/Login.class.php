...
class Login {
    ...
    /**
     * <b>Obter Erro:</b> Retorna um array associativo com uma mensagem e um tipo de erro SOS_.
    ...
    public function CheckLogin() {
        if (empty($_SESSION['userlogin']) || $_SESSION['userlogin']['cod_tipo_usuario'] < $this->Level):
    ...
    private function setLogin() {
        if (!$this->Email || !$this->Senha || !Check::Email($this->Email)):
            $this->Error = ['Informe seu E-mail e senha para efetuar o login!', SOS_INFOR];
            $this->Result = false;
        elseif (!$this->getUser()):
            $this->Error = ['Os dados informados não são compatíveis!', SOS_ALERT];
            $this->Result = false;
        elseif ($this->Result['cod_tipo_usuario'] < $this->Level):
            $this->Error = ["Desculpe {$this->Result['nome']}, você não tem permissão para acessar esta área!", SOS_ERROR];
            $this->Result = false;
        else:
            $this->Execute();
        endif;
    ...
    private function getUser() {
        $this->Senha = sha1(md5($this->Senha));
    ...
    private function Execute() {
        ...
        $this->Error = ["Olá {$this->Result['username']}, seja bem vindo(a). Aguarde redirecionamento!", SOS_ACCEPT];
        $this->Result = true;
    }
